...
public class CommandBO {
    ...
    //通过构造函数传递进来命令
    public CommandBO(String commandStr) {
        //常规判断
        if (commandStr != null && commandStr.length() != 0) {
            //根据分隔符号拆分出执行符号
            String[] complexStr = commandStr.split(CommandBO.DIVIDE_FLAG);
            //第一个参数是执行符号
            ...
            for (int i = 1; i < complexStr.length; i++) {
                ...
                //包含前缀符号，认为是参数
                if (str.indexOf(CommandBO.PREFIX) == 0) {
                    this.paramList.add(str.replace (CommandBO.PREFIX, "").trim());
                } else {
